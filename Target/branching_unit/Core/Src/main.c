...
/*
 * Function: branching_unit
 * ------------------------
 * Controls the branching logic to either jump to the user application or the bootloader.
 */
void branching_unit() {
	// Check if the RTC Backup data Register 0 contains the flag value (0x0000)
	if (HAL_RTCEx_BKUPRead(&hrtc, RTC_BKP_DR0) == 0x0000) {
		// Check if the user application address is not empty
		if (*USER_APP_ADDRESS != 0xFFFFFFFF) {
			// Jump to the user application
			Jump_To_User_App();
		}
		// First time boot or no user application found
		Jump_To_Bootloader();
	} else {
		// Flag found in RTC Backup data Register 0
		Jump_To_Bootloader();
	}
}
...
/*
 * Function: Jump_To_Bootloader
 * ----------------------------
 * Prepares and jumps to the bootloader.
 */
void Jump_To_Bootloader() {
	// Turn on the Green LED
	HAL_GPIO_WritePin(GPIOG, GPIO_PIN_13, GPIO_PIN_SET);

	// Print a message indicating the start of the bootloader
...
/*
 * Function: Jump_To_User_App
 * --------------------------
 * Prepares and jumps to the user application.
 */
void Jump_To_User_App() {
	...
	// Read the value of the MSP (Main Stack Pointer) from the user application address
	uint32_t msp_value = *(volatile uint32_t*)0x08010000;

	// Set the MSP to the value read from the user application address
	__set_MSP(msp_value);

	// Read the address of the application reset handler from the user application address + 4 bytes
	uint32_t resethandler_address = *(volatile uint32_t*)(0x08010000 + 4);

	// Assign the application reset handler address to the function pointer
	app_reset_handler = (void*)resethandler_address;

	// Call the application reset handler to jump to the user application
	app_reset_handler();
}
...
//	HAL_FLASH_Program(FLASH_TYPEPROGRAM_BYTE, ADDRESS, 0);
//
//
//	HAL_FLASH_Program(FLASH_TYPEPROGRAM_BYTE , ADDRESS + 1, 0);
//
//	HAL_FLASH_Lock();
